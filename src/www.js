  ...
  <div>
      ...
      <b-container>
        ...
        <b-row>
            ...
            <b-col lg="7">
              <h2>STORE & CREATOR <h6 style="display: inline-block;">
                  ...
                  <a href="javascript:void(0)" v-on:click="searchCreator()"><i class="fas fa-address-book"></i></a>
    ...
    methods: {
      ...
      _createAvatar () {
        ...
        if(data!=null)
          this.wallet.sendTx(aMgr.address,this.common.password,0,data,(e)=>{this.common.state=false;this.common.message=e;},(h)=>{this.common.state=true;this.common.message="Tx:"+h;},(r)=>{this.common.state=true;this.common.message="Success";});
      },
      searchAvatar () {
        if(this.wallet.web3&&this.wallet.isAddress()) {
          let that    = this;
          let address = this.wallet.web3.utils.padLeft(this.wallet.address(),64);
          let topics  = 'topic0='+aMgr.abi[11]['signature']+'&topic2='+address+'&topic3='+address+'&topic2_3_opr=or';
          that.showModal('Your avatar stores','md','dark','now loading...');
          that.wallet.logs(aMgr.address,topics,(data)=>{
            let list = [];
            for(let i = 0 ; i < data.length ; i++) {
              let store = '0x'+data[i].topics[1].toString().slice(-40).toLowerCase();
              ...
              if(owner==this.wallet.address().toLowerCase())
                list.push({store:store,owner:owner,from:from});
              else {
            ...
            if(list.length>0) {
              for(let i = list.length-1 ; i >= 0  ; i--)
                that.modal.items.push({'key':list[i].store,'root':that});
      ...
      searchCreator() {
        if(this.wallet.web3&&this.wallet.isAddress()) {
          // todo : loding my store
          this.$refs.refModalCreator.show();
          this.showModal('Your Creator Accounts','md','dark','todo : temp.......');
        } else
          this.showModal('Alert','sm','warning',"Login wallet please.");
      },
      //-------------------------------- Creator
...
Vue.component('store-item', {
  props: ['item'],
  template: '<b-input-group size="sm" class="mb-1"><b-form-input :value="item.key" readonly></b-form-input><b-input-group-append><b-btn size="sm" v-on:click="item.root.avatarUI.address=item.key;item.root.showEditAvatar(true);"><i class="fas fa-file-invoice-dollar"></i></b-btn><b-btn size="sm" v-on:click="item.root.avatarUI.address=item.key;item.root.showEditAvatar(false);"><i class="fas fa-file-signature"></i></b-btn></b-input-group-append></b-input-group>'
});
