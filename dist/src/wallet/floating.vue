...
<script>
  ...
  wallet.pushContract(window.config.manager,window.config.address);
  ...
  export default {
    ...
    computed: {
      ...
      stateTo: function () {
        return wallet.web3?wallet.isAddress(this.withdrawal.to)&&wallet.address()&&wallet.address().toLowerCase()!=this.withdrawal.to.toLowerCase():false;
      },
